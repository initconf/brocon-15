...
export {
	...
	type irc_user: record {
	 ...
	 redef record irc_channel += {
	 #type irc_channel: record {
                c_name: string &optional;         # channel name
                c_users: set[string] &optional; # users in channel
                c_ops: set[string] &optional ;     # channel operators
                c_type: string &optional;         # channel type
                c_modes: string &optional;        # channel modes
                c_topic: string &optional;        # channel topic
        };
	...
	global active_users: table[string] of irc_user &read_expire = 6 hrs &redef ; 
		#&expire_func = expired_user &redef;

	# Active channels, indexed by channel name.
	global active_channels: table[string] of irc_channel &read_expire = 6 hrs &redef; 
...
#	{
##print log_file, fmt("%.6f #%s %s", network_time(), conn_list[c$id], msg);
#print  fmt("%.6f #%s %s", network_time(), conn_list[c$id], msg);
#	}
...
event irc_request(c: connection, is_orig: bool, prefix: string,
	...
	{
	check_connection(c);
        c$irc_detailed$irc_event = fmt ("irc_request");
...
event irc_message(c: connection, is_orig: bool, prefix: string,
	...
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_message");	
	...
	if ( command == /.*:$/ )
		...
		event protocol_violation(c, Analyzer::ANALYZER_IRC, aid, "broken server response");
		return;
		}
	...
	if ( command !in ignore_in_other_msgs )
		log_activity(c, fmt("other server PRIVMSG %s: %s",
					prefix, context));
	}
...
event irc_quit_message(c: connection, is_orig: bool, nick: string, message: string)
	...
	c$irc_detailed$irc_event = fmt ("irc_quit_message");
...
{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("%s", msg_type);
...
event irc_privmsg_message(c: connection, is_orig: bool, source: string, target: string,
				message: string)
	{
	check_message(c, source, target, message, "message");
	}

event irc_notice_message(c: connection, is_orig: bool, source: string, target: string,
				message: string)
	{
	check_message(c, source, target, message, "notice");
	}

event irc_squery_message(c: connection, is_orig: bool, source: string, target: string,
				message: string)
	{
...
event irc_join_message(c: connection, is_orig: bool, info_list: irc_join_list)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_join_message");
...
event irc_part_message(c: connection, is_orig: bool, nick: string,
	...
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_part_message");
...
event irc_nick_message(c: connection, is_orig: bool, who: string, newnick: string)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_nick_message");

	log_activity(c, fmt("%s nick name to '%s'",
				who == "" ?  "changing" : fmt("user '%s' changing", who), newnick));
	}

event irc_invalid_nick(c: connection, is_orig: bool)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_invalid_nick");
	log_activity(c, "changing nick name failed");

	}

event irc_network_info(c: connection, is_orig: bool, users: count, services: count,
			servers: count)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_network_info");

	log_activity(c, fmt("network includes %d users, %d services, %d servers",
				users, services, servers));
	}

event irc_server_info(c: connection, is_orig: bool, users: count, services: count,
			servers: count)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_server_info");
...
event irc_channel_info(c: connection, is_orig: bool, chans: count)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_channel_info");
	log_activity(c, fmt("network includes %d channels", chans));
	}

event irc_who_line(c: connection, is_orig: bool, target_nick: string, channel: string,
			user: string, host: string, server: string,
			nick: string, params: string, hops: count,
			real_name: string)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_who_line");
...
event irc_who_message(c: connection, is_orig: bool, mask: string, oper: bool)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_who_message");
...
event irc_whois_message(c: connection, is_orig: bool, server: string, users: string)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_whois_message");
...
event irc_whois_user_line(c: connection, is_orig: bool, nick: string,
	...
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_whois_user_line");
...
event irc_whois_operator_line(c: connection, is_orig: bool, nick: string)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_whois_operator");
...
event irc_whois_channel_line(c: connection, is_orig: bool, nick: string, chans: string_set)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_whois_channel");
...
event irc_oper_message(c: connection, is_orig: bool, user: string, password: string)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_oper_message");
...
event irc_oper_response(c: connection, is_orig: bool, got_oper: bool)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_oper_response");
	log_activity(c, fmt("user %s operator status",
				got_oper ? "received" : "did not receive"));
	}

event irc_kick_message(c: connection, is_orig: bool, prefix: string, chans: string,
			users: string, comment: string)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_kick_message");
...
event irc_error_message(c: connection, is_orig: bool, prefix: string, message: string)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_error_message");
...
event irc_invite_message(c: connection, is_orig: bool, prefix: string,
	...
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_invite_message");
...
event irc_mode_message(c: connection, is_orig: bool, prefix: string, params: string)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_mode_message");
...
event irc_squit_message(c: connection, is_orig: bool, prefix: string,
	...
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_squit_message"); 
...
event irc_names_info(c: connection, is_orig: bool, c_type: string, channel: string,
			users: string_set)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_names_info");
...
event irc_dcc_message(c: connection, is_orig: bool, prefix: string, target: string,
			dcc_type: string, argument: string,
			address: addr, dest_port: count, size: count)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_dcc_message");
...
event irc_channel_topic(c: connection, is_orig: bool, channel: string, topic: string)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_channel_topic");
...
event irc_password_message(c: connection, is_orig: bool, password: string)
	{
	check_connection(c);
	c$irc_detailed$irc_event = fmt ("irc_password_message");
